...
# 2.  that matches a string that has an a followed by zero or more b's.
...
# 6.  that matches a string that has an a followed by two to three 'b'.
def check_string6(string):
    pattern = re.compile(r'^ab{2,3}$')
    return 'valid' if pattern.match(string) else 'invalid'


# 7.  to find sequences of lowercase letters joined with a underscore.
